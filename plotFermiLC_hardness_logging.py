...
class ExitOnExceptionHandler(logging.StreamHandler):
    def emit(self, record):
        super().emit(record)
        if record.levelno in (logging.CRITICAL, logging.CRITICAL):
            raise SystemExit(1)

logging.basicConfig(handlers=[ExitOnExceptionHandler()], level=logging.INFO)
plotFermiLChardness=logging.getLogger('plotFermiLC')      
...
if FITS is None:
    plotFermiLChardness.critical("object files not found")
...
try:
    ...
    plotFermiLChardness.critical("Error opening file: "+FITS)
...
if cfg.hardness_ratio_plot:
    ...
    i300_1000=np.where(np.array(tbdata['UL_300_1000'])==False)[0]
    i1000_300000=np.where(np.array(tbdata['UL_1000_300000'])==False)[0]
    i=list(set(i300_1000).intersection(i1000_300000))
    
    # FLUXES
    flux_300_1000=(np.array(tbdata['FLUX_300_1000']))[i]
    flux_1000_300000=(np.array(tbdata['FLUX_1000_300000']))[i]
    hrformula=lambda a,b: a/b
    hr=hrformula(flux_1000_300000,flux_300_1000)
    
    #getting time of UL==False events and time uncertainty
    
    t=51910+np.array(x[i])/(60*60*24)
    dx=dx[i]

    #UNDCERATINTIES IN FLUXES

    flux_300_1000_err=(np.array(tbdata['ERROR_300_1000']))[i]
    flux_1000_300000_err=(np.array(tbdata['ERROR_1000_300000']))[i]
    hreformula=lambda a,b,c,d: np.sqrt(((1/b)*c)**2+((-a/(b**2))*d)**2)
    hre=hreformula(flux_1000_300000,flux_300_1000,flux_1000_300000_err,flux_300_1000_err)
...
if cfg.Crab_flux:
    ...
    if not cfg.quiet:
        plotFermiLChardness.info("Plotting Crab flux {}: {} ph cm-2 s-1".format(F,cf))
...
if cfg.Average:
    if in_3FGL:
        if not cfg.quiet:
            plotFermiLChardness.info("Plotting object average flux {}: {:.2e} ph cm-2 s-1".format(F,flux_3FGL))
...
if cfg.Swift:
    ...
    if swift_file is None:
        if not cfg.quiet:
            plotFermiLChardness.info("Could not download Swift lightcurve for "+ object)           

    else:  # "-S given w file so use
        if not cfg.quiet: plotFermiLChardness.info("Loading Swift data from "+swift_file)
...
if cfg.png:
    if cfg.hardness_ratio_plot:
        png_file=root_filename+"_harndess_ratio.png"
    else:
        png_file=root_filename+".png"
    if not cfg.quiet: plotFermiLChardness.info("Saving "+png_file)
    plt.savefig(png_file)


if cfg.pdf:
    if cfg.hardness_ratio_plot:
        pdf_file=root_filename+"_hardness_ratio.pdf"
    else:
        pdf_file=root_filename+".pdf"
    if not cfg.quiet: plotFermiLChardness.info("Saving "+pdf_file)
...
if cfg.remove_FITS:
    if not cfg.quiet: plotFermiLChardness.info("Removing "+FITS)
    os.remove(FITS)
